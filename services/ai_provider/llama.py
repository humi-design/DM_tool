...
LLAMA_MODELS = [
    ModelInfo(
        ...
        cost_per_input_token=0.0007,
        cost_per_output_token=0.0028,
    ),
    ModelInfo(
...
class LlamaProvider(BaseAIProvider):
    """Meta Llama AI provider (via Together AI or other compatible APIs)."""
    
    BASE_URL = "https://api.together.xyz/v1"
    
    def __init__(self, config: ProviderConfig):
        self._provider_type = ProviderType.LLAMA
    ...
    def _initialize_client(self) -> None:
        """Initialize the HTTP client for Llama."""
        headers = {
        ...
        self._client.provider_type = "llama"
    
    def get_available_models(self) -> List[ModelInfo]:
        """Return available Llama models."""
        return LLAMA_MODELS
    ...
    @log_provider_call("chat", "llama", "model")
    async def chat(
        self,
        ...
        **kwargs
        ...
        model = model or self.config.model or "llama-3.3-70b-instruct"
    ...
    async def stream(
        ...
        **kwargs
        ...
        """Send a streaming chat completion request to Llama."""
        model = model or self.config.model or "llama-3.3-70b-instruct"
    ...
    def _extract_usage(
        ...
        model_info = next(
            (m for m in LLAMA_MODELS if m.id == model),
            LLAMA_MODELS[0]
        )
        cost = 0.0
